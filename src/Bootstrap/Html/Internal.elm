module Bootstrap.Html.Internal exposing (BtnParam, btnc, btncNoevent, colOffset)
...
colOffset gridsize colspan offset =
    let
        prefix =
            "col" ++ String.cons '-' gridsize ++ "-"
    in
    if offset > 0 then
        prefix ++ String.fromInt colspan ++ String.cons ' ' prefix ++ "offset-" ++ String.fromInt offset

    else
        prefix ++ String.fromInt colspan
